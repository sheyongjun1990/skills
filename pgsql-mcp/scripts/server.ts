...
class ConfigManager {
  ...
  private loadPasswordFile(filePath: string): void {
    try {
  ...
  private resolvePassword(id: string, ds: any, configFilePath: string): string {
  ...
  getAllDataSources(): Array<{ id: string; config: DataSourceConfig }> {
    return Array.from(this.configs.entries()).map(([id, config]) => ({ id, config }));
  }

  // Smart matching based on hint
  matchDataSource(hint: string): string | undefined {
